...
//! Objective-C object registered with `NSApp` at runtime, so both halves have
//! to agree: `NSMessage` names the selector implemented below.
//!
...
fn capture(app: &AppHandle, text: String) {
    ...
    // A Service fires while another app is frontmost; the note is only useful
    // if the user can see it being written.
...
/// Offer the app's Services to the rest of the system. Must run on the main
/// thread, after `NSApp` exists.
pub fn register(app: &AppHandle) {
    ...
    let _ = app.clone().run_on_main_thread(move || {
        ...
        // The menu is cached per app; without this the item only appears after
        // the next login or `lsregister` sweep.

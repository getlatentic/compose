...
**Status:** v3.1 production architecture draft  
...
**Supersedes:** `vellum-local-first-production-spec-v3.md`.

---

## 0. What changed from v3

v3 made the right product-level shift:

```txt
Local first before collaboration.
Clean Markdown files before portable sidecars.
Comments as local document annotations and LLM context handles.
App-owned metadata by default.
```

v3.1 tightens the parts that were still too soft for production:
...
The product stance remains local-first, but the core editor is now specified like a production system rather than a proof of concept.
...
no confusing hidden project files by default
```
...
### 2.6 App metadata is app-owned by default

The user’s vault should not contain `.vellum/` by default.

```txt
Vault folder:
  user Markdown files only
...
The app can later offer optional portable metadata modes, but the default should protect non-technical users from deleting or mis-syncing app internals.
...
### 3.1 Do not put `.vellum/` in the vault by default

A hidden sidecar inside the working folder creates practical problems:
...
So v1 should not create it automatically.
...
Do not inject app-specific comment IDs into Markdown by default:

```md
<!-- vellum-comment:abc123 -->
```

Inline IDs are useful for portability, but they pollute source and make the editor feel proprietary. Keep them optional.

### 3.5 Offer portable metadata explicitly

Some users will want comments, history, and AI threads to travel with the vault.

Offer explicit modes:

```txt
Default mode:
  app-managed metadata outside vault

Portable metadata file:
  .vellum.sqlite inside vault

Portable metadata folder:
  .vellum/ inside vault

Export mode:
  JSON or Markdown export of comments/AI threads/history
```

The UI copy should be clear:

```txt
“Keep metadata private on this computer”
“Make this vault portable”
“Export comments and AI chats”
```
...
## 17. Portable metadata options

Default behavior keeps metadata outside the vault.
...
Git-tracked metadata if explicitly desired
portable vaults
```

### 17.3 Portable metadata folder
...
Good for larger vaults but more visible/confusing.
...
### R12: Portable metadata and export

Build:

```txt
export comments/AI threads
import metadata
optional .vellum.sqlite
optional .vellum/ folder
metadata recovery UI
```

Exit gate:

```txt
Users can move or back up metadata intentionally without making hidden sidecars the default.
```
...
```txt
No `.vellum/` sidecar by default.
Metadata lives in app-owned storage by default.
...
8. Should `.vellum.sqlite` be the only portable mode, or should `.vellum/` also exist for large indexes?
9. How should the app recover metadata if the user reinstalls the app but keeps the Markdown vault?
10. Should AI provider credentials live in app config, keychain, or both?
11. Should stable `BlockId` be generated randomly at first observation or deterministically from structural fingerprints?
12. How aggressively should the dirty semantic parse window expand for ambiguous Markdown constructs?
